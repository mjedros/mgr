...
void ProcessingImage::binarize(const unsigned int &minimum,
    ...
    try {
        ...
        cl::Image2D image_out(openCLManager->context, CL_MEM_WRITE_ONLY,
                              formatOut, image.cols, image.rows, 0);
        openCLManager->queue.enqueueWriteImage(image_in, CL_TRUE, origin,
                                               region, 0, 0, image.data);
...
void ProcessingImage::process(cl::Kernel &kernel, cl::Image2D &image_in,
    ...
    timeSum += std::chrono::duration_cast<std::chrono::milliseconds>(
                   end - start).count();
}
